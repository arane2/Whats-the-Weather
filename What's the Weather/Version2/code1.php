...
<?php
		...
		// Connect Database to check if the pair is present
		/*DATABASE CONNECT to store*/
		$servername = "localhost";
		$username = "root";
		$password = "";
		// Create connection
		$conn = new mysqli($servername, $username, $password, "mywaypoints");
		// Check connection
		...
		$sql = "SELECT Current, Destination,JSON,weaJSON FROM waypoints WHERE current='".$xs."' AND destination='".$ys."'";
		...
		$sql = 'INSERT INTO waypoints (Current,Destination, JSON, weaJSON) VALUES ("'.$xs.'","'.$ys.'","'.mysqli_real_escape_string($conn,$homepage).'","'.mysqli_real_escape_string($conn,$weather).'")';
